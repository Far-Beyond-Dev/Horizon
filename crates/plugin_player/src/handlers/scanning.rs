...
mod tests {
    ...
    #[test]
    fn test_scan_validation_success() {
        let scan_data = ScanData {
            ship_class: "Interceptor".to_string(),
            hull_integrity: 85.0,
            shield_strength: 75.0,
            cargo_manifest: vec!["Ammunition".to_string()],
            pilot_level: 10,
            energy_signature: 120.0,
            weapon_systems: vec!["Lasers".to_string()],
        };
        
        assert!(validate_scan_request(1, &scan_data).is_ok());
    }
    
    #[test]
    fn test_scan_validation_invalid_hull() {
        let scan_data = ScanData {
            ship_class: "Interceptor".to_string(),
            hull_integrity: 150.0, // Invalid - over 100%
            shield_strength: 75.0,
            cargo_manifest: vec![],
            pilot_level: 10,
            energy_signature: 120.0,
        ...
        assert!(validate_scan_request(1, &scan_data).is_err());
    }
    
    #[test]
    fn test_scan_validation_too_much_cargo() {
        let cargo = (0..60).map(|i| format!("Item {}", i)).collect(); // Too many items
        let scan_data = ScanData {
    ...
    #[test]
    fn test_ship_class_generation() {
        let interceptor_data = generate_scan_data_for_ship_class("Interceptor", 15);
        assert_eq!(interceptor_data.ship_class, "Interceptor");
        assert!(interceptor_data.energy_signature > 100.0);
        assert!(interceptor_data.cargo_manifest.is_empty());
        
        let freighter_data = generate_scan_data_for_ship_class("Freighter", 20);
        assert_eq!(freighter_data.ship_class, "Freighter");
        assert!(!freighter_data.cargo_manifest.is_empty());
        assert!(freighter_data.energy_signature < 100.0);
    }
}