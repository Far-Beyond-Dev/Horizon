...
mod tests {
    ...
    #[test]
    fn test_message_validation_success() {
        assert!(validate_message_content("Hello, requesting docking clearance", "general").is_ok());
    }
    
    #[test]
    fn test_message_validation_empty() {
        assert!(validate_message_content("", "general").is_err());
    }
    
    #[test]
    fn test_message_validation_too_long() {
        let long_message = "a".repeat(501);
        assert!(validate_message_content(&long_message, "general").is_err());
    }
    
    #[test]
    fn test_message_validation_invalid_channel() {
        assert!(validate_message_content("Hello", "invalid_channel").is_err());
    }
    
    #[test]
    fn test_communication_behavior_emergency() {
        let behavior = determine_communication_behavior("emergency", 1, None);
        match behavior {
            CommunicationBehavior::Emergency { range, priority } => {
                assert!(range > 300.0);
                assert!(priority);
            },
            _ => panic!("Expected emergency behavior"),
        }
    }
    
    #[test]
    fn test_communication_behavior_private_with_target() {
        let behavior = determine_communication_behavior("private", 1, Some(2));
        match behavior {
            CommunicationBehavior::Direct { target } => {
                assert_eq!(target, 2);
            },
            _ => panic!("Expected direct behavior"),
        }
    }
}