...
impl TestGamePlugin {
    ...
    fn load_recipes_json(&mut self) {
        // Corrected format with direct u32 IDs
    ...
    "#;
        ...
        // Second recipe with corrected format
        let recipe2_json_str = r#"
    {
        ...
        "data": {
            "recipe": {
                ...
                "name": "Iron Sword",
                "ingredients": [
                    { "item_id": 104, "quantity": 2, "name": "Iron Ingot" },
                    { "item_id": 102, "quantity": 1, "name": "Stick" }
                ],
                "main_product": { "item_id": 203, "quantity": 1, "name": "Iron Sword" },
                "byproducts": [
                    { "item_id": 302, "quantity": 1, "name": "Iron Shavings" }
                ]
            }
        }
    }
    ...
    async fn send_crafting_outcome(
        &self,
        ...
        let crafting_outcome = json!({
            ...
            "data": {
                "player_id": player_id,
                "outcome": {
                    "recipe_id": 1,  // Direct u32 value
                    "main_product": {
                        "item_id": 201,  // Direct u32 value
                        "quantity": 1,
                        "name": "Wooden Pickaxe"
                    },
                    "byproducts": [],
        ...
        context
            .emit_event(EventNamespace::new("core"), Box::new(event))
            .await
            .map_err(|e| {
                PluginError::ExecutionError(format!("Failed to emit crafting outcome event: {}", e))
            })
    }

    // Example of requesting recipe info
    async fn request_recipe_info(
        &self,
        context: &dyn ServerContext,
        player_id: &str,
        recipe_id: u32,
        ...
        let get_recipe_info = json!({
            ...
            "data": {
                "player_id": player_id,
                "recipe_id": recipe_id  // Direct u32 value
            }
        })
        .to_string();
        ...
        let result = context
            ...
            .await
            .map_err(|e| {
                PluginError::ExecutionError(format!("Failed to emit get recipe info event: {}", e))
            });
...
impl Plugin for TestGamePlugin {
    ...
    async fn initialize(&mut self, context: &dyn ServerContext) -> Result<(), PluginError> {
        ...
        context.log(
            LogLevel::Info,
            "TestGamePlugin initializing and loading recipes JSON",
        );
        ...
        for recipe_msg_json in &self.recipes_json {
            ...
            context
                .emit_event(EventNamespace::new("recipesmith"), Box::new(event))
                .await
                .map_err(|e| {
                    PluginError::ExecutionError(format!("Failed to emit recipe JSON event: {}", e))
                })?;
            context.log(
                LogLevel::Info,
                "Sent AddOrUpdateRecipe JSON message to RecipeSmith",
            );
        }
        ...
        let data = self.request_recipe_info(context, "player123", 1).await?;

        println!("Recipe info request data: {:#?}", data);
    ...
    async fn handle_event(
        ...
        if event_id.namespace.0 == "recipesmith" {
            context.log(
                LogLevel::Info,
                &format!("Received RecipeSmith event: {}", event_id.event_type),
            );
